...
import { Creator, Recipe } from '../../types/database';
...
const EditRecipePage = () => {
  ...
  const [formData, setFormData] = useState<Omit<Recipe, 'id' | 'created_at' | 'updated_at'>>({
    title: '',
    description: '',
    category: '',
    cook_time: '',
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [recipeData, creatorsData] = await Promise.all([
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please login to edit recipes</p>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await recipesApi.updateRecipe(recipeId!, recipe);
      navigate('/admin/recipes');
    } catch (error) {
